...
chatForm.addEventListener('submit', async function (e) {
    e.preventDefault(); const message = userInput.value.trim(); if (!message || isLoading) return; if (welcomeScreen) { welcomeScreen.style.display = 'none'; } addMessage(message, 'user'); const filePathMatch = message.match(/([A-Za-z]:[\\/][^\s'"]+\.[a-zA-Z0-9]+)/); if (filePathMatch) { const filePath = filePathMatch[1]; await loadFileInEditor(filePath); } userInput.value = ''; userInput.style.height = 'auto'; updateCharCount(); isLoading = true; sendBtn.disabled = true; sendBtn.classList.add('loading'); const loadingEl = addLoadingMessage(); try {
        const requestBody = { message }; if (currentEditorFile && codeEditor && codeEditor.value) { requestBody.file_path = currentEditorFile; requestBody.file_content = codeEditor.value; } const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(requestBody) }); const data = await response.json(); loadingEl.remove(); if (data.error) { addMessage('Error: ' + data.error, 'bot'); showToast('Error: ' + data.error, 'error'); } else {
            const isStreamAction = data.pending_action === 'stream_to_editor';
            ...
            if (isStreamAction && data.action_data && data.action_data.code) {
                // If a path is provided but not current, load it first
                if (data.action_data.path && data.action_data.path !== currentEditorFile) {
                    await loadFileInEditor(data.action_data.path);
                }
                showPendingChanges(data.action_data.code, data.action_data.changes || 'Code changes');
            }
            saveChatSession();
        }
    } catch (error) { loadingEl.remove(); addMessage('Error: Connection error. Please try again.', 'bot'); showToast('Connection error', 'error'); } finally { isLoading = false; sendBtn.disabled = false; sendBtn.classList.remove('loading'); userInput.focus(); }
});
...
function showPendingChanges(newCode, summary) { if (!codeEditor) return; const originalCode = codeEditor.value; openEditor(); showDiffModal(originalCode, newCode, currentEditorFile || 'Untitled', summary); }
function acceptChanges() { if (!pendingNewCode) return; codeEditor.classList.remove('pending-changes'); changeButtons.classList.add('hidden'); setEditorModified(true); editorStatus.textContent = '✓ Changes accepted - Save to apply'; editorStatus.className = 'editor-status saved'; showToast('Changes accepted! Click Save to write to file.', 'success'); pendingNewCode = null; originalEditorCode = null; }
function rejectChanges() { if (!originalEditorCode) return; codeEditor.value = originalEditorCode; codeEditor.classList.remove('pending-changes'); changeButtons.classList.add('hidden'); updateLineNumbers(); updateEditorInfo(); editorStatus.textContent = 'Changes rejected'; editorStatus.className = 'editor-status'; showToast('Changes rejected', 'info'); pendingNewCode = null; originalEditorCode = null; }
...
function showToast(message, type = 'info') { if (!toastContainer) return; const toast = document.createElement('div'); toast.className = `toast ${type}`; const icon = type === 'success' ? 'Info' : type === 'error' ? 'Error' : 'Info'; toast.innerHTML = `<span class="toast-icon"></span>${message}`; toastContainer.appendChild(toast); setTimeout(() => { toast.remove(); }, 3000); }
...
function getFileIcon(item) { if (item.isDirectory) return ''; const ext = item.extension; const icons = { '.py': '', '.js': '', '.ts': '', '.html': '', '.css': '', '.json': '', '.md': '', '.txt': '', '.jpg': '', '.jpeg': '', '.png': '', '.gif': '', '.pdf': '', '.zip': '', '.rar': '', '.exe': '', '.sql': '', '.env': '' }; return icons[ext] || ''; }
...
function generateSimpleDiff(oldCode, newCode) {
    ...
    for (let i = 0; i < Math.min(maxLines, 100); i++) { // Increase limit to 100 lines
...
async function acceptDiffChanges() {
    ...
    if (!filePath || filePath === 'Untitled') {
        // No file path - just apply to editor without saving
        if (codeEditor && newCode) {
            codeEditor.value = newCode;
            updateLineNumbers();
            updateEditorInfo();
            setEditorModified(true);
            editorStatus.textContent = '✓ Changes applied - Save to write to file';
            editorStatus.className = 'editor-status saved';
            showToast('Changes applied! Click Save to write to file.', 'success');
        }
        closeDiffModal();
        return;
    }
    ...
    if (diffAcceptBtn) {
        ...
        diffAcceptBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/></svg>Saving...';
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            if (codeEditor) {
                codeEditor.value = newCode;
                originalContent = newCode; // Update original so it's not marked as modified
                updateLineNumbers();
                updateEditorInfo();
                ...
                // Visual feedback: scroll to top and pulse highlight
                codeEditor.scrollTop = 0;
                codeEditor.classList.add('editor-pulse');
                setTimeout(() => codeEditor.classList.remove('editor-pulse'), 2000);
            }

            // Add the bot's final confirmation message to chat
            if (data.response) {
                addMessage(data.response, 'bot');
                saveChatSession();
            }

            showToast(data.message || 'Changes saved to file!', 'success');
        } else {
        ...
        if (diffAcceptBtn) {
            ...
            diffAcceptBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"/></svg>Accept & Save';
        }
        closeDiffModal();
    }
}
...
async function rejectDiffChanges() {
    try {
        // Notify backend to resume graph with rejection
...
window.handleConfirmAction = async function (action, actionData) {
    if (action === 'accept' && actionData && actionData.code) {
        ...
        showDiffModal(originalCode, actionData.code, currentEditorFile || 'Untitled', actionData.changes || 'Code changes');
